...
abstract class Kernel extends QMatrix {
    ...
    static double edit(String x, String y) {
        int len1 = x.length();
        int len2 = y.length();
        int[][] row = new int[2][len1 + 1];
        ...
        for (int j = 0; j < len2; j++) {
            ...
            for (int i = 0; i < len1; i++) {
                row[now][i + 1] = Math.min(Math.min(row[old][i] + (x.charAt(i) == y.charAt(j) ? 0 : 1), row[old][i + 1] + 1), row[now][i] + 1);
            }
            now = 1 - now;
            old = 1 - old;
        }
...
public class svm {
    ...
    public static CustomKernel customKernel = null;

    public static void setCustomKernel(CustomKernel customKernel) {
        svm.customKernel = customKernel;
    }
    ...
    private static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm) {
        ...
        for (i = 0; i < l; i++) {
            perm[start[data_label[i]]] = i; //start[class] denotes position of current instance
            ++start[data_label[i]];
        }
        start[0] = 0;
        for (i = 1; i < nr_class; i++) {
    ...
    static final String kernel_type_table[] = {
        "linear", "polynomial", "rbf", "sigmoid", "precomputed", "custom"
    };
    ...
    public static String svm_check_parameter(svm_problem prob, svm_parameter param) {
        ...
        if (kernel_type != svm_parameter.CUSTOM) {
